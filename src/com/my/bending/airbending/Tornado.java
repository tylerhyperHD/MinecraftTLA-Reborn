package com.my.bending.airbending;
...
public class Tornado {
	...
	public Tornado(Player player) {
		...
		Block b = GetTargetBlock.getTargetBlock(null, (int) range);
        final Location loc = b.getLocation();
		loc.setY(loc.getY() - 1. / 10. * height);
	...
	public boolean progress() {
		...
		if (Tools
				.isRegionProtectedFromBuild(player, Abilities.AirBlast, loc)) {
			instances.remove(player.getEntityId());
			return false;
		}
		rotateTornado();
	...
	private void rotateTornado() {
		Block origin = GetTargetBlock.getTargetBlock(null, (int) range);
		...
		if (loc.getBlock().getType() != Material.AIR) {
			loc.setY(origin.getY() - 1. / 10. * height);

			for (Entity entity : Tools.getEntitiesAroundPoint(loc, height)) {
