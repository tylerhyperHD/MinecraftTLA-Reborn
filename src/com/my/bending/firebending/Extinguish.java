...
public class Extinguish {
	...
	public Extinguish(Player player) {
		...
		double range = Tools.firebendingDayAugment(defaultrange, player.getWorld());
		if (Tools.isMeltable(GetTargetBlock.getTargetBlock(null, (int) range))) {
			new HeatMelt(player);
			return;
		}
		double radius = Tools.firebendingDayAugment(defaultradius, player.getWorld());
		for (Block block : Tools.getBlocksAroundPoint(GetTargetBlock.getTargetBlock(null, (int) range).getLocation(), radius)) {
